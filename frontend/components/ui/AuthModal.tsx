...
export default function AuthModal({ onClose, initialTab = "login" }: Props) {
  ...
  const inp = "w-full bg-slate-800 border border-slate-600 rounded-lg px-3 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition-colors disabled:opacity-50";
  const lbl = "block text-[10.5px] font-bold text-slate-400 uppercase tracking-wide mb-1.5";

  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50"
      onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-slate-900 border border-slate-700 rounded-2xl p-7 w-full max-w-md relative shadow-2xl">
        <button onClick={onClose} className="absolute top-4 right-4 text-slate-400 hover:text-slate-200 p-1 rounded-md hover:bg-slate-700 transition-colors">
          <X size={16} /></button>
        <div className="text-center mb-6">
          <div className="w-11 h-11 bg-indigo-500 rounded-xl flex items-center justify-center text-white font-bold text-lg font-display mx-auto mb-3">R</div>
        ...
        <div className="flex bg-slate-800 rounded-lg p-1 gap-1 mb-5">
          {(["login", "signup"] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)} className={`flex-1 py-2 text-sm font-semibold rounded-md transition-all ${tab === t ? "bg-indigo-500 text-white" : "text-slate-400 hover:text-slate-200"}`}>
        ...
        {tab === "login" ? (
          <div className="flex flex-col gap-3">
            ...
            <button onClick={doLogin} disabled={loading} className="w-full bg-indigo-500 hover:bg-indigo-600 disabled:opacity-60 text-white font-semibold py-2.5 rounded-lg transition-colors mt-1 flex items-center justify-center gap-2">
          ...
          <div className="flex flex-col gap-3">
            ...
            <button onClick={doSignup} disabled={loading} className="w-full bg-indigo-500 hover:bg-indigo-600 disabled:opacity-60 text-white font-semibold py-2.5 rounded-lg transition-colors mt-1 flex items-center justify-center gap-2">
