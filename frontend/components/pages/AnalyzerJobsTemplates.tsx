...
export function AnalyzerPage() {
  ...
  const keywords = [
    ...
    ...partialMatches.map((match) => ({
      ...
      color: match.conf === "high" ? "bg-lime-500" : "bg-lime-500/80",
  ...
  return (
    <div className="max-w-5xl grid grid-cols-1 lg:grid-cols-[1fr_256px] gap-4 items-start">
      <div className="space-y-4">
        {/* Main card */}
        <div className="bg-surface/60 border border-violet-900/50 rounded-2xl p-5">
          <div className="flex items-start justify-between mb-4">
            ...
            <button onClick={() => { showToast("Re-running analysis…", "info"); setTimeout(() => showToast("Analysis refreshed!", "ok"), 2200); }}
              className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-semibold border border-violet-800/50 text-slate-300 rounded-lg hover:bg-surface-2 transition-all">
              <RefreshCw size={11} /> Re-analyze
            </button>
          </div>

          {/* Sub-tabs */}
          <div className="flex border-b border-violet-900/50 mb-4">
            {(["score", "optimized", "keywords"] as const).map(t => (
              <button key={t} onClick={() => setTab(t)}
                className={`px-3.5 py-2 text-xs font-bold border-b-2 transition-all capitalize font-display ${tab === t ? "text-fuchsia-400 border-fuchsia-500" : "text-slate-500 border-transparent hover:text-slate-300"}`}>
            ...
            <div className="space-y-5">
              <div>
                ...
                <div className="space-y-1.5">
                  {exactMatches.map(m => (
                    <div key={m.skill} className="flex items-center gap-2.5 px-3 py-2 bg-surface-2/40 rounded-lg">
              ...
              <div>
                <div className="text-xs font-bold text-white font-display mb-2">Partial Matches <span className="text-lime-400 font-normal">({partialMatches.length} inferred)</span></div>
                <div className="space-y-1.5">
                  {partialMatches.map(m => (
                    <div key={m.skill} className="flex items-center gap-2.5 px-3 py-2 bg-surface-2/40 rounded-lg">
                      <AlertCircle size={13} className="text-lime-400 shrink-0" />
              ...
              <div>
                <div className="text-xs font-bold text-white font-display mb-2">Missing <span className="text-lime-400 font-normal">({missing.length} gaps)</span></div>
                <div className="flex flex-wrap gap-1.5">
                  {missing.map(m => (
                    <span key={m} className="text-xs font-semibold px-2.5 py-1 rounded-full bg-lime-500/10 text-lime-400 border border-lime-500/20">{m}</span>
                  ))}
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2 mb-4">
                {optimizedBullets.length > 0 ? optimizedBullets.map((b, i) => (
                  <div key={i} className="flex items-start gap-2 px-3 py-2.5 bg-surface-2/40 rounded-lg text-xs text-slate-300 leading-relaxed">
                    <span className="text-emerald-400 text-xs mt-0.5 shrink-0">↗</span>{b}
                  </div>
                )) : (
                  <div className="px-3 py-2.5 bg-surface-2/40 rounded-lg text-xs text-slate-400">Run analysis to generate optimized resume bullets.</div>
                )}
              </div>
              ...
              <button onClick={() => showToast("Optimized bullets applied to resume!", "ok")}
                className="flex items-center gap-1.5 px-3.5 py-1.5 bg-fuchsia-500 hover:bg-fuchsia-600 text-white text-xs font-semibold rounded-lg transition-all">
        ...
        <div className="bg-surface/60 border border-violet-900/50 rounded-2xl p-5">
          ...
          <div className="space-y-2.5">
            {(selectedAnalysis?.suggestions?.length ? selectedAnalysis.suggestions : [
              ...
              <div key={s.area} className="px-3.5 py-2.5 border-l-2 border-fuchsia-500 bg-surface-2/40 rounded-r-lg">
                <div className="text-[10px] font-bold text-fuchsia-400 uppercase tracking-wide font-display mb-1">{s.area}</div>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-surface/60 border border-violet-900/50 rounded-2xl p-5">
          ...
          <button onClick={() => { showToast("Re-running analysis…", "info"); setTimeout(() => showToast("Score updated!", "ok"), 2000); }}
            className="flex items-center gap-1.5 w-full justify-center px-3 py-2 bg-fuchsia-500 hover:bg-fuchsia-600 text-white text-xs font-semibold rounded-lg transition-all">
            <RefreshCw size={12} /> Re-analyze
          </button>
        </div>

        {/* Job switcher */}
        <div className="bg-surface/60 border border-violet-900/50 rounded-2xl p-4">
          ...
          <div className="space-y-1.5">
            {jobs.length > 0 ? jobs.map((job) => {
              ...
              const color = scoreValue === undefined ? "bg-slate-500" : scoreValue >= 70 ? "bg-emerald-500" : scoreValue >= 50 ? "bg-lime-500" : "bg-lime-500";
              ...
              return (
                <div key={job.id} onClick={() => {
                  ...
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer transition-all ${isActive ? "bg-fuchsia-500/12 border border-fuchsia-500/25" : "hover:bg-surface-2/50"}`}>
...
export function JobsPage() {
  ...
  const scoreBg = (s?: number) => !s ? "bg-surface-2/50 text-slate-400 border-violet-800/50" : s >= 70 ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" : s >= 50 ? "bg-lime-500/10 text-lime-400 border-lime-500/20" : "bg-lime-500/10 text-lime-400 border-lime-500/20";
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          <div className="flex flex-col gap-1">
            ...
            <input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="e.g. Backend Engineer"
              className="bg-ink-2 border border-violet-800/50 rounded-lg px-3 py-2 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-fuchsia-500 transition-colors" />
          </div>
          <div className="flex flex-col gap-1">
          ...
          <div className="col-span-2 flex flex-col gap-1">
            ...
            <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} rows={6}
              ...
              className="bg-ink-2 border border-violet-800/50 rounded-lg px-3 py-2 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-fuchsia-500 transition-colors resize-y w-full leading-relaxed" />
          </div>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={submit} className="flex items-center gap-1.5 px-4 py-2 bg-fuchsia-500 hover:bg-fuchsia-600 text-white text-sm font-semibold rounded-lg transition-all">
            <CheckCircle size={13} /> Save & Run Analysis
          </button>
          <button onClick={() => setForm({ title: "", company: "", description: "" })} className="px-4 py-2 border border-violet-800/50 text-slate-300 text-sm font-semibold rounded-lg hover:bg-surface-2 transition-all">Clear</button>
        ...
        {jobs.map(job => (
          <div key={job.id} className="bg-surface/50 border border-violet-900/50 rounded-xl p-4 relative">
            ...
            <div className="flex gap-2.5 mb-3 items-start">
              <div className="w-8 h-8 rounded-lg bg-fuchsia-500 flex items-center justify-center text-xs font-bold text-white font-display shrink-0">{job.company[0]}</div>
              <div>
            ...
            <div className="flex gap-2">
              <button onClick={() => showToast(`Loading ${job.company} analysis…`, "info")}
                className="flex-1 py-1.5 text-xs font-semibold bg-fuchsia-500 hover:bg-fuchsia-600 text-white rounded-lg transition-all">
                {job.analyzed ? "View Analysis" : "Run Analysis"}
              </button>
              <button onClick={() => { removeJob(job.id); showToast("Deleted", "ok"); }}
                className="p-1.5 text-slate-500 hover:text-lime-400 hover:bg-lime-500/10 rounded-lg transition-all">
                <XCircle size={14} />
              </button>
            </div>
          </div>
...
const TEMPLATES_INFO = [
  { id: "modern" as const, name: "Modern", style: "Two-column · blue header", badge: "Most popular", badgeColor: "bg-emerald-500/10 text-emerald-400" },
    ...
    <div className="max-w-5xl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {TEMPLATES_INFO.map(t => (
          <div key={t.id} onClick={() => setTemplate(t.id)}
            className={`bg-surface/60 border-2 rounded-2xl overflow-hidden cursor-pointer transition-all ${activeTemplate === t.id ? "border-fuchsia-500 shadow-glow" : "border-violet-900/60 hover:border-fuchsia-500/40"}`}>
            <div className="bg-ink-2 p-3 h-48 overflow-hidden">
            ...
            <div className="p-3 flex items-start justify-between">
              ...
              {activeTemplate === t.id && <CheckIcon size={14} className="text-fuchsia-400 shrink-0 mt-0.5" />}
            </div>
          </div>
        ))}
      </div>
      <div className="text-center">
        <button onClick={() => goTo("builder")}
          className="px-6 py-2.5 bg-fuchsia-500 hover:bg-fuchsia-600 text-white font-semibold rounded-xl transition-all text-sm">
