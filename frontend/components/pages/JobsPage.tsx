...
export default function JobsPage() {
  ...
  const scoreStyle = (s?: number) =>
    !s ? "bg-slate-700/50 text-slate-400 border-slate-600"
      ...
      <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-5 mb-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setForm({ title: "", company: "", description: "" })}
            className="px-4 py-2 border border-slate-600 text-slate-300 text-sm font-semibold rounded-lg hover:bg-slate-700 transition-all">Clear</button>
              ...
              <div key={job.id} className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-4 relative">
                ...
                <div className="flex gap-2">
                  <button onClick={() => analyze(job.id)} disabled={isAnalyzing}
                    className="flex-1 py-1.5 text-xs font-semibold flex items-center justify-center gap-1.5 bg-indigo-500 hover:bg-indigo-600 disabled:opacity-60 text-white rounded-lg transition-all">
